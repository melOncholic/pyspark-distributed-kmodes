...
def custom_cdist(XA, XB, dtypes, W=None):
    """
    Customized gower distance
    """
    if XA.shape[1] != XB.shape[1]:
        raise ValueError
    cols = XA.shape[1]
    W = np.ones((cols,)) if W is None else W
    wij = sum(W)
    Y = []
    for col in xrange(cols):
        if np.issubdtype(dtypes[col], np.number):
            t = np.linalg.norm(np.subtract(XA[:, col].astype(np.float32), XB[:, col].astype(np.float32)), axis=1)
            t = t / np.max(t) if np.max(t) != 0 else t
        else:
            t = np.asarray(np.not_equal(XA[:, col], XB[:, col]).transpose().astype(np.uint8))[0]
        # print(XA[:, col], XB[:, col], t)
        Y.append(t * W[col])
    return sum(np.asmatrix(Y)) / wij
...
def matching_dissim(a, b):
    ...
    """
    centers = pd.DataFrame(b, index=len(b))
    XA = np.asmatrix(centers.values)
    dtypes = list(centers.dtypes)
    XB = np.asmatrix(pd.DataFrame(a, index=len(a)).values)
    return custom_cdist(XA, XB, dtypes, W=None)
